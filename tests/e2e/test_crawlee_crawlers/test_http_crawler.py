...
async def test_http_crawler(make_actor: MakeActorFunction, run_actor: RunActorFunction) -> None:
    actor = await make_actor(
        label='crawl-http',
        source_files={'server.py': _TEST_SERVER_PY, 'src/main.py': _HTTP_CRAWLER_MAIN_PY},
    )
    run_result = await run_actor(actor)
    await _verify_crawler_results(actor, run_result, 'HttpCrawler')
