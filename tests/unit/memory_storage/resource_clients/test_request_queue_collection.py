...
async def test_get_or_create(request_queues_client: RequestQueueCollectionClient) -> None:
    ...
    assert os.path.exists(os.path.join(request_queues_client.request_queues_directory, rq_name, '__metadata__.json')) is False
    rq_info = await request_queues_client.get_or_create(name=rq_name)
    assert rq_info['name'] == rq_name
    assert os.path.exists(os.path.join(request_queues_client.request_queues_directory, rq_name, '__metadata__.json')) is True
