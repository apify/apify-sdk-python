...
async def test_request_reclaim_functionality(
    ...
    request2 = await poll_until_condition(
...
async def test_request_reclaim_with_forefront(
    ...
    # at the queue head (eventually-consistent API state), even in single mode, so poll until it does.
    next_request = await poll_until_condition(
        rq.fetch_next_request,
        lambda result: result is not None,
        timeout=60,
        poll_interval=5,
...
async def test_complex_request_objects(
    ...
    complex_request = Request.from_url(
    ...
    await rq.add_request(complex_request)
...
async def test_request_deduplication_edge_cases(request_queue_apify: RequestQueue) -> None:
    ...
    for url, expected_duplicate in urls_and_deduplication_expectations:
        ...
        result = await poll_until_condition(
            lambda url=url: rq.add_request(url),
            lambda result: result is not None,
            timeout=60,
            poll_interval=5,
        )
...
async def test_request_queue_metadata_another_client(
    ...
    metadata = await poll_until_condition(
        rq.get_metadata,
        lambda m: m.total_request_count >= 1,
        timeout=60,
        poll_interval=5,
    )
...
async def test_cache_initialization(apify_token: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with Actor:
        ...
        try:
            ...
            metadata = await poll_until_condition(
                _get_rq_metadata,
                lambda m: (m.stats.read_count or 0) - (stats_before.read_count or 0) >= len(requests),
                timeout=60,
                poll_interval=5,
            )
            ...
            assert (stats_after.read_count or 0) - (stats_before.read_count or 0) == len(requests)
