...
- <ApiLink to="class/ApifyGracefulStopExtension">`apify.scrapy.extensions.ApifyGracefulStopExtension`</ApiLink> - A Scrapy [extension](https://docs.scrapy.org/en/latest/topics/extensions.html) that stops the crawl when the Actor run is aborted gracefully. A graceful abort gives the run a short grace period before it's killed, and the extension uses that time to let the requests in flight finish and get marked as handled. For details, see [Dealing with imminent migration to another host](#dealing-with-imminent-migration-to-another-host).
