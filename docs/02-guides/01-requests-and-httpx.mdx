...
<!-- TODO: link the correct template -->

## Requests

[`requests`](https://requests.readthedocs.io) is one of the most popular Python libraries for doing HTTP requests.
To use it in your actors, there is no special configuration needed.
Just put `requests` in your `requirements.txt` file, reinstall dependencies if you're running the actor locally,
...
To use the Apify Proxy with `requests`,
...
To use it in your actors, there is no special configuration needed.
Just put `httpx` in your `requirements.txt` file, reinstall dependencies if you're running the actor locally,
and you're good to go.
...
To use the Apify Proxy with `httpx`,
you can just generate a proxy URL through [`Actor.create_proxy_configuration()`](../../reference/class/Actor#create_proxy_configuration),
...
async def main():
    async with Actor:
        ...
        async with httpx.AsyncClient(proxies=proxy_url) as httpx_client:
            response = httpx_client.get(f'http://example.com/{i}'),
            print(response)
```
