...
Like the Apify platform, the SDK remembers a key for three minutes, so a later repeat is charged again. A repeat that names a different event is skipped with a warning and reports zero.
